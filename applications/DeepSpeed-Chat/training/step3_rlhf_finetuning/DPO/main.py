# dpo implementation goes here!

#!/usr/bin/env python
# Copyright (c) Microsoft Corporation.
# SPDX-License-Identifier: Apache-2.0

# DeepSpeed Team and me!
...
from dschat.rlhf.dpo_trainer import DeepSpeedDPOTrainer
# from dschat.rlhf.rlhf_engine import DeepSpeedRLHFEngine
from dschat.rlhf.dpo_engine import DeepSpeedDPOEngine
...
class CustomCollator(DataCollatorWithPadding):
    def __init__(self, fields_to_collate, tokenizer):
        self.tokenizer = tokenizer
        self.fields_to_collate = fields_to_collate

    def __call__(self, examples):
        selected_fields = [{key: example[key] for key in self.fields_to_collate} for example in examples]

        collated = super().__call__(selected_fields)
        dl = collated['input_ids'].shape[1]
        collated['label_tensor'] = torch.stack([F.pad(example['label_tensor'], (0, dl - len(example['label_tensor'])), 'constant', 0) for example in examples])
        return collated

def create_datasets(args, tokenizer, train_phase="DPO"):
    unsupervised_training_enabled = args.unsupervised_dataset_name and args.unsupervised_dataset_config_name
    prompt_train_dataset, _ = create_prompt_dataset(
        args.local_rank, args.data_path, args.data_split,
        args.data_output_path, train_phase, args.seed, tokenizer,
        args.max_prompt_seq_len, reload=True)
    print(f"Prompt train dataset length: {len(prompt_train_dataset)}")
    print(f"Prompt train dataset[0]: {prompt_train_dataset[0]}")
    ...
    data_collator = CustomCollator(['input_ids', 'attention_mask'], tokenizer)
...
def main():
    ...
    prompt_train_dataloader, unsupervised_train_dataloader, num_total_iters = create_datasets(
        args=args, tokenizer=tokenizer, train_phase="DPO")

    # RLHF engine is responsible for creating models, loading checkpoints, ds-initialize models/optims/lr-schedulers
    dpo_engine = DeepSpeedDPOEngine( # TODO(SELF): WRITE DEEPSPEED DPO ENGINE IN UTILS (done)
    ...
    if args.enable_mixed_precision_lora:
        ...
        dpo_engine.actor.optimizer.quantize_nontrainable_params()
        print_rank_0("Mixed Precision ZeRO++ enabled")

    dpo_trainer = DeepSpeedDPOTrainer
    trainer = dpo_trainer(dpo_engine, args)
    ...
    if torch.distributed.get_rank() == 0:
        run = wandb.init(project="sft_amps_llama7b", config = {"learning_rate": args.actor_learning_rate,
    ...
    for epoch in range(args.num_train_epochs):
        print_rank_0(
            f"Beginning of Epoch {epoch+1}/{args.num_train_epochs}, Total Batches {min(len(prompt_train_dataloader), len(unsupervised_train_dataloader))}",
            args.global_rank)
        training_start = time.time()
        dpo_engine.actor.train()
        for step, (batch_prompt) in enumerate(prompt_train_dataloader):

            input_ids = batch_prompt['input_ids'][:, :-1]
            attention_mask = batch_prompt['attention_mask'][:, :-1]
            labels = batch_prompt['input_ids'][:, 1:].clone()

            batch_prompt = to_device(batch_prompt, device)
            policy_out = trainer.get_cumlog_probs(dpo_engine.actor, 
                                                  input_ids,
                                                  attention_mask,
                                                  labels)
            
            ref_out = trainer.get_cumlog_probs(dpo_engine.ref,
                                                input_ids,
                                                attention_mask,
                                                labels)

            actor_loss, rewards = trainer.train_dpo(policy_out, ref_out, batch_prompt['simple_labels'])
            ...
            print_rank_0(
                f'Epoch: {epoch} | Step: {step} | Actor Loss: {actor_loss}, Rewards: {rewards}',
                args.global_rank)
            print_rank_0(
                "-------------------------------------------------------------------------------------",
                args.global_rank)
            ...
            if args.actor_gradient_checkpointing:
                dpo_engine.actor.gradient_checkpointing_disable()
    ...
    if args.output_dir is not None:
        ...
        if args.enable_ema:
            dpo_engine.actor_ema = convert_lora_to_linear_layer(
                dpo_engine.actor_ema)
        ...
        if torch.distributed.get_rank() == 0:
            ...
            if args.enable_ema:
                save_hf_format(dpo_engine.actor_ema,
                               tokenizer,
                               args,
        ...
        if args.actor_zero_stage == 3:
            ...
            if args.enable_ema:
                save_zero_three_model(dpo_engine.actor_ema,
                                      global_rank=args.global_rank,
                                      save_dir=os.path.join(
